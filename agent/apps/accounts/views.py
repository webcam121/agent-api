...
class RetrieveGiverDetailForReceiverView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        latest_paid_user_plan = UserPlan.objects.filter(receiver=gift_receiver, is_active=True).order_by('-created_at').first()

        if not latest_paid_user_plan:
            return Response({"detail": "Receiver does not have an paid membership"}, status=status.HTTP_404_NOT_FOUND)

        # Retrieve the giver user associated with the active user plan
        giver_user = latest_paid_user_plan.giver.user
        giver_detail = {
            "giver_email": giver_user.email,
            "giver_first_name": giver_user.first_name,
            "giver_last_name": giver_user.last_name,
        }

        return Response({"giver": giver_detail}, status=status.HTTP_200_OK)
...
class InviteUserForGiverView(APIView):
    permission_classes = [IsAuthenticated, IsGiftGiver]
    serializer_class = InviteUserSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            gift_receiver = get_object_or_404(GiftReceiver, user__id=receiver_id)

            latest_paid_user_plan = UserPlan.objects.filter(receiver=gift_receiver, giver=gift_giver, is_active=True).order_by('-created_at').first()

            # Check if there is paid user plan or not
            if not latest_paid_user_plan:
                return Response({"detail": "You don’t have permission to invite others"},
                ...
                if custom_user:
                    ...
                    # Make the above giver to receiver's giver
                    new_giver = GiftGiver.objects.filter(user=custom_user).first()
                    gift_receiver.gift_giver.add(new_giver)
                    gift_receiver.save()
                    ...
                    # Send email to the new giver
                    send_invitation_email_to_user_from_giver.delay(user_id=custom_user.pk, giver_id=gift_giver.pk, receiver_id=gift_receiver.pk, daily_update=daily_update)
                    return Response(status=status.HTTP_200_OK)
                # In case the user is new to Agent
                else:
                    send_invitation_email_to_nonuser_from_giver.delay(email=email, first_name=first_name, last_name=last_name, phone_number=phone_number, giver_id=gift_giver.pk, receiver_id=gift_receiver.pk, daily_update=daily_update)
                    return Response(status=status.HTTP_200_OK)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class AcceptInvitationTokenView(APIView):
    serializer_class = AcceptInvitationSerializer

    def post(self, request, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            invitation_token_key = serializer.validated_data.get('invitation_token_key')
            invitation_token = get_object_or_404(InvitationToken, key=invitation_token_key)
            receiver = invitation_token.receiver
            daily_update = invitation_token.daily_update
            user_email = invitation_token.user_email
            user_first_name = invitation_token.user_first_name
            user_last_name = invitation_token.user_last_name
            user_phone_number = invitation_token.user_phone_number

            if receiver:
                try:
                    user = CustomUser.objects.create(email=user_email, is_giver=True, first_name=user_first_name, last_name=user_last_name, phone_number=user_phone_number)
                    giver = get_object_or_404(GiftGiver, user=user)
                    receiver.gift_giver.add(giver)
                    receiver.save()

                    daily_update_obj, created = DailyUpdate.objects.get_or_create(gift_giver=giver, gift_receiver=receiver)
                    daily_update_obj.is_daily_update = daily_update
                    daily_update_obj.save()
                    ...
                    return Response({"token": token.key, "receiver_id": receiver.pk}, status=status.HTTP_200_OK)
...
class DailyUpdateModifyForGiverView(APIView):
    permission_classes = [IsAuthenticated, IsGiftGiver]
    serializer_class = DailyUpdateModifySerializer

    def post(self, request, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            daily_update = serializer.validated_data.get('daily_update')
            giver_user = self.request.user
            giver = GiftGiver.objects.filter(user=giver_user).first()
            receiver_id = kwargs.get('gift_receiver_id')
            receiver = get_object_or_404(GiftReceiver, pk=receiver_id)

            if not receiver.gift_giver.filter(pk=giver.pk).exists():
                return Response("You don't have permission for the receiver", status=status.HTTP_403_FORBIDDEN)

            daily_update_obj, created = DailyUpdate.objects.get_or_create(gift_giver=giver,
    ...
    def get(self, request, *args, **kwargs):
        ...
        daily_update_obj = get_object_or_404(DailyUpdate, gift_giver=giver, gift_receiver=receiver)
        return Response({"daily_update": daily_update_obj.is_daily_update}, status=status.HTTP_200_OK)


class DailyUpdateModifyForReceiverView(APIView):
    permission_classes = [IsAuthenticated, IsGiftReceiver]
    serializer_class = DailyUpdateModifySerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            daily_update = serializer.validated_data.get('daily_update')
            receiver_user = self.request.user
            receiver = GiftReceiver.objects.filter(user=receiver_user).first()
            giver_id = kwargs.get('gift_giver_id')
            giver = get_object_or_404(GiftGiver, pk=giver_id)

            if not receiver.gift_giver.filter(pk=giver.pk).exists():
                return Response("You don't have permission for the giver", status=status.HTTP_403_FORBIDDEN)
