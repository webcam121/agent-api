...
from agent.apps.notifications.tasks import send_reminder_email_to_giver
                ...
                if call_status == 'no-answer':
                    ...
                    if not call_session:
                        if schedule_logs.count() > 3:
                            ...
                            # answer the call
                            send_reminder_email_to_giver.delay(receiver.pk)
                            ...
                            ETATask.objects.create(
                                task_name='run_scheduled_call',
                                run_at=run_at_time,
                                task_kwargs=json.dumps({'call_schedule_id': schedule_log.call_schedule.id}),
                            )
