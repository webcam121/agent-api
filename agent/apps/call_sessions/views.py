...
from ..eta_tasks.models import ETATask
...
class EndCallSessionAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        call_session.save()
        # tasks.summarize_call_session.apply_async((call_session.pk,), countdown=30)
        run_at_time = datetime.now() + timedelta(seconds=30)
        ETATask.objects.create(
            task_name='summarize_call_session',
            run_at=run_at_time,
            task_kwargs=json.dumps({'call_session_id': call_session.pk}),
        )
